...
df2 %>% group_by(Sex) %>% summarise(n_Age = length(Age))

```

```{r}
# Function that counts number of observation of col_count based on group/s 
count_keys <- function(data, col_count, ...) {
	data <- data.table(data)
	paste_formula <- paste0("length(", col_count, ")", collapse = NULL)
	data %>% group_by(...) %>% summarize(count = eval(parse(text = (paste_formula))))
}

count_keys(data = df2, col_count = "Age", Sex)
...
df2[, n_Age := length(Age), by = .(Sex)]


	data <- data[, (count) := eval(parse(text = (paste_formula))), by = eval((...))] 
	data
